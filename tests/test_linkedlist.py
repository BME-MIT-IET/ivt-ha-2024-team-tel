...
class TestCopyRandomPointer(unittest.TestCase):
    def test_copy_random_pointer_v1(self):
        # Assume RandomListNode has a constructor taking `label` and `next`
        head = RandomListNode(1, RandomListNode(2, RandomListNode(3)))
        head.random = head.next.next
        head.next.random = head

        copied_list = copy_random_pointer_v1(head)
        self.assertNotEqual(head, copied_list)
    ...
    def test_copy_random_pointer_v2(self):
        ...
        copied_list = copy_random_pointer_v2(head)
        self.assertNotEqual(head, copied_list)
        self.assertEqual(copied_list.label, head.label)
        self.assertEqual(copied_list.random.label, head.random.label)
#added 
...
#added 
class TestFirstCyclicNode(unittest.TestCase):
